...
const HiAnimeContinueWatching = ({ limit, showMoreButton = false, onMoreClick }: HiAnimeContinueWatchingProps) => {
  const dispatch = useAppDispatch()
  const hianimeContinueWatching = useAppSelector(selectHiAnimeContinueWatching)
  const [showAll, setShowAll] = useState(false)

  const displayedItems = limit && !showAll ? hianimeContinueWatching.slice(0, limit) : hianimeContinueWatching
  const hasMore = limit ? hianimeContinueWatching.length > limit : false

  const handleRemove = (itemId: string, event: React.MouseEvent) => {
    event.preventDefault()
    event.stopPropagation()
    console.log('Removing HiAnime from continue watching:', itemId)
    dispatch(removeFromHiAnimeContinueWatching(itemId))
  }
  ...
  const getMediaUrl = (item: any) => {
    return `/hianime/${item.id}`
  }

  if (hianimeContinueWatching.length === 0) {
    return null
  }
...
export default HiAnimeContinueWatching
